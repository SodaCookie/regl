  ...
  let start () => {
    ...
    let getTileSide puzzle::puzzle tile::tile position::(GCoord {x: px, y: py}) => {
      ...
      let GCoord {x: tx, y: ty} =
        getTileCenter (centerPoint puzzleSize::puzzleSize position::(toGameCoord tile.position));
      if (py < ty - 7) {
        Bottom
      } else if (px < tx - 7) {
        Left
    ...
    let onMouseMove puzzle::puzzle gameState::gameState x::x y::y => {
      let mousePos = GCoord {x: x, y: windowSize - y};
      let rec loop i =>
        switch gameState.lineEdge {
        | None => ()
        | Some lineEdge =>
    ...
    let render puzzle::puzzle gameState::gameState time => {
      ...
      | Some lineEdge =>
        ...
        myDrawCircle radius::(lineWeight / 2) color::Color.red position::lineEdge
      }
    };
